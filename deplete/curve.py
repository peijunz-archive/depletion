        ...
        for i in range(l):
            ...
            if abs(sig) < err:
                ...
                delta = normalize_angle(
                    former.end_angle(), latter.start_angle())
...
class PCurve(list):
    ...
    def __init__(self, cur, shift=0):
        list.__init__([])
        l = len(cur)
        self.extend([seg.start(), seg, (i + 1) % l]
                    for i, seg in enumerate(cur))
